...
// UnmarshalYAML implements custom unmarshaling for IfImageNotFound to handle polymorphic Actions field
func (a *IfImageNotFound) UnmarshalYAML(unmarshal func(interface{}) error) error {
...
func (a *IfImageNotFound) Validate(ab *ActionBuilder) error {
	...
	for i, action := range a.Actions {
		if err := action.Validate(ab); err != nil {
			return fmt.Errorf("IfImageNotFound (%s) -> nested action %d: %w", a.Template, i+1, err)
		}
	}
...
func (a *IfImageNotFound) Build(ab *ActionBuilder) *ActionBuilder {

	step := Step{
		name: fmt.Sprintf("IfImageNotFound (%s)", a.Template),
		execute: func(bot BotInterface) error {
			...
			if err := subBuilder.executeSteps(bot.Context(), bot); err != nil {
				return fmt.Errorf("IfImageNotFound (%s) -> nested action failed: %w", a.Template, err)
			}

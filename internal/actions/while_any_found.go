...
type WhileAnyImagesFound struct {
	Templates   []string     `yaml:"templates"`
	MaxAttempts int          `yaml:"max_attempts"`
	Actions     []ActionStep `yaml:"actions"`
}

// UnmarshalYAML implements custom unmarshaling for WhileAnyImagesFound to handle polymorphic Actions field
func (a *WhileAnyImagesFound) UnmarshalYAML(unmarshal func(interface{}) error) error {
...
func (a *WhileAnyImagesFound) Validate(ab *ActionBuilder) error {
	if a.MaxAttempts < 0 {
	...
	for i, action := range a.Actions {
		if err := action.Validate(ab); err != nil {
			return fmt.Errorf("WhileAnyImagesFound (%s) -> nested action %d: %w", a.Templates, i+1, err)
		}
	}
...
func (a *WhileAnyImagesFound) Build(ab *ActionBuilder) *ActionBuilder {
	step := Step{
		name: fmt.Sprintf("WhileAnyImagesFound (%s)", a.Templates),

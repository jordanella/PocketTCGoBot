...
type IfAnyImagesFound struct {
	Templates []string     `yaml:"templates"`
	Actions   []ActionStep `yaml:"actions"`
}

// UnmarshalYAML implements custom unmarshaling for IfAnyImagesFound to handle polymorphic Actions field
func (a *IfAnyImagesFound) UnmarshalYAML(unmarshal func(interface{}) error) error {
...
func (a *IfAnyImagesFound) Validate(ab *ActionBuilder) error {
	...
	for i, action := range a.Actions {
		if err := action.Validate(ab); err != nil {
			return fmt.Errorf("IfAnyImagesFound (%s) -> nested action %d: %w", a.Templates, i+1, err)
		}
	}
...
func (a *IfAnyImagesFound) Build(ab *ActionBuilder) *ActionBuilder {
	step := Step{
		name: fmt.Sprintf("IfAnyImagesFound (%s)", a.Templates),
		execute: func(bot BotInterface) error {
			...
			if err := subBuilder.executeSteps(bot.Context(), bot); err != nil {
				return fmt.Errorf("IfAnyImagesFound (%d) failed: %w", len(a.Templates), err)
			}

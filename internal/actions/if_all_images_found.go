...
type IfAllImagesFound struct {
	Templates []string     `yaml:"templates"`
	Actions   []ActionStep `yaml:"actions"`
}

// UnmarshalYAML implements custom unmarshaling for IfAllImagesFound to handle polymorphic Actions field
func (a *IfAllImagesFound) UnmarshalYAML(unmarshal func(interface{}) error) error {
...
func (a *IfAllImagesFound) Validate(ab *ActionBuilder) error {
	...
	for i, action := range a.Actions {
		if err := action.Validate(ab); err != nil {
			return fmt.Errorf("IfAllImagesFound (%s) -> nested action %d: %w", a.Templates, i+1, err)
		}
	}
...
func (a *IfAllImagesFound) Build(ab *ActionBuilder) *ActionBuilder {
	step := Step{
		name: fmt.Sprintf("IfAllImagesFound (%s)", a.Templates),

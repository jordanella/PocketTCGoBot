...
// LoadFromDefinition pre-populates the wizard with existing pool definition
func (w *UnifiedPoolWizard) LoadFromDefinition(def *accountpool.UnifiedPoolDefinition) {
	w.description = def.Description

	// Convert queries
	w.queries = make([]QueryConfig, len(def.Queries))
	for i, q := range def.Queries {
		w.queries[i] = QueryConfig{
			Name:    q.Name,
			Filters: q.Filters,
			Sort:    q.Sort,
			Limit:   q.Limit,
	...
	// Copy slices
	w.includes = append([]string{}, def.Include...)
	w.excludes = append([]string{}, def.Exclude...)
	w.watchedPaths = append([]string{}, def.WatchedPaths...)

	// Copy config
	if def.Config.SortMethod != "" {
		w.sortMethod = def.Config.SortMethod
	}
	w.retryFailed = def.Config.RetryFailed
	w.maxFailures = def.Config.MaxFailures
	w.refreshInterval = def.Config.RefreshInterval
}
